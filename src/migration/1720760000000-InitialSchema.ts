...
export class InitialSchema1720760000000 implements MigrationInterface {
    ...
    private async upgradeSynchronizedSchema(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`category\` DROP INDEX \`FK_32b856438dffdc269fa84434d9f\``);
        await queryRunner.query(`ALTER TABLE \`category\` ADD UNIQUE INDEX \`IDX_category_name_user\` (\`name\`, \`userId\`)`);
        await queryRunner.query(`ALTER TABLE \`category\` ADD CONSTRAINT \`FK_category_user\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);

        await queryRunner.query(`ALTER TABLE \`image\` DROP FOREIGN KEY \`FK_dc40417dfa0c7fbd70b8eb880cc\``);
        await queryRunner.query(`ALTER TABLE \`image\` DROP INDEX \`FK_dc40417dfa0c7fbd70b8eb880cc\``);
        await queryRunner.query(`ALTER TABLE \`image\` MODIFY \`description\` varchar(500) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`image\` MODIFY \`userId\` varchar(36) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`image\` ADD INDEX \`IDX_image_user\` (\`userId\`)`);
        await queryRunner.query(`ALTER TABLE \`image\` ADD CONSTRAINT \`FK_image_user\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` DROP INDEX \`IDX_96306bf482c0acf805cd0fd688\``);
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` DROP INDEX \`IDX_f67ec53757f47fb0c38f396c37\``);
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` ADD INDEX \`IDX_image_category_image\` (\`imageId\`)`);
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` ADD INDEX \`IDX_image_category_category\` (\`categoryId\`)`);
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` ADD CONSTRAINT \`FK_image_category_image\` FOREIGN KEY (\`imageId\`) REFERENCES \`image\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`image_categories_category\` ADD CONSTRAINT \`FK_image_category_category\` FOREIGN KEY (\`categoryId\`) REFERENCES \`category\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
